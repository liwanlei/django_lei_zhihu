...
class UserWidgetAdmin(object):
    ...
    list_display = ('widget_type', 'page_id', 'coustmer')
    list_filter = ['coustmer', 'widget_type', 'page_id']
    list_display_links = ('widget_type',)
    user_fields = ['coustmer']
    hidden_menu = True
...
class Dashboard(CommAdminView):
    ...
    def get_context(self):
        new_context = {
            ...
            "?coustmer=%s&page_id=%s&_redirect=%s" % (self.user.id, self.get_page_id(), urlquote(self.request.get_full_path()))
        }
